@ Implementation of Shabal: ARM, big-endian, ARM-only opcodes.
@
...
@ -----------------------------------------------------------------------
	...
	.file   "shabal_tiny_armeb_armonly.s"
	.text

@ byteswap(bb)
@    bb   pointer to a 64-byte buffer
@
@ This function expects its argument in v1, not a1. Registers a1, a2 and
@ a4 may be modified. It byteswaps in place the 16 words at the specified
@ address. The buffer must be word-aligned.
@
	.align  2
	.code   32
...
byteswap:
	add     a4, v1, #64
.L0b:
	ldr     a1, [a4, #-4]          @ Load next word.
	eor     a2, a1, a1, ror #16    @ Byteswap word (part 1).
	mov     a1, a1, ror #8         @ Byteswap word (part 2).
	bic     a2, a2, #0xFF0000      @ Byteswap word (part 3).
	eor     a1, a1, a2, lsr #8     @ Byteswap word (part 4).
	str     a1, [a4, #-4]!         @ Store swapped word, update a4.
	cmp     a4, v1                 @ Decrement loop counter.
	bne     .L0b                   @ Loop 16 times.
	mov     pc, lr                 @ Return.

@ Implementation of Shabal: ARM, big-endian, mixed ARM/Thumb code.
@
...
@ -----------------------------------------------------------------------
	...
	.file   "shabal_tiny_armeb_mixed.s"
	.text
...
.L0b:
	ldr     a1, [v1]               @ Load next word.
	mov     a2, #4                 @ Set sub-loop counter.
.L1b:
	strb    a1, [v1]               @ Store next byte.
	add     v1, #1                 @ Increment destination.
	lsr     a1, a1, #8             @ Shift word.
	sub     a2, #1                 @ Decrement sub-loop counter.
	bne     .L1b                   @ Loop 4 times.
	sub     a4, #1                 @ Decrement loop counter.
	bne     .L0b                   @ Loop 16 times.

	sub     v1, #64                @ Restore original v1.
	bx      lr                     @ Return.
...
.L1c:
	...
	add     v1, #180               @ Set v1 to address of C[0].
	bl      byteswap               @ Byteswap C words.
	add     v1, #64                @ Set v1 to address of W field.

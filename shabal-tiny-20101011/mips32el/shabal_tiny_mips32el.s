# Implementation of Shabal: MIPS, little-endian, 32-bit.
#
...
$Lmc1:
	...
	lwr     $v1, 0($a1)            # Load source word (part 1) (delay slot).
	lwl     $v1, 3($a1)            # Load source word (part 2).
	addiu   $a1, $a1, 4            # Increment source pointer.
	addiu   $t1, $t1, -4           # Decrement data length.
	swr     $v1, 0($t0)            # Store word (part 1).
	swl     $v1, 3($t0)            # Store word (part 2).
	b       $Lmc1                  # Loop.
...
$Lsc3:
	...
	lw      $t1, 252($a0)          # Load output size (bits).
